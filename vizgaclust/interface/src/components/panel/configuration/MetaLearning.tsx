...
class MetaLearning extends React.Component<MetaLearningProps, MetaLearningState>{
    ...
    render(){
        ...
        const label = "Select clustering algorithm and meric"
        return (
            <ParamBox>
                <Heading margin="none" level={5} data-tip={label} data-place={"right"}> Algorithm & Fitness Function </Heading>
                {/*<RadioButtonGroup*/}
                {/*    name="radio"*/}
                {/*    options={ options }*/}
                {/*    value={value}*/}
                ...
                {/*/>*/}
                {!metaLearning && (
                    <Button icon={<Configure />} label="Set Configurations" onClick={this.onOpenModal} color={"#2a3142"}/>
                )}
                {not_meta_options && (
                    <Layer
                        position="right"
